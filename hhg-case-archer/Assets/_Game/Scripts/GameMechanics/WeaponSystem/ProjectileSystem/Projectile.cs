...
namespace _Game.GameMechanics
{
    public class Projectile : MonoBehaviour
    {
        ...
        //     _time = 0f;
        //
        //     if (!CalculateLaunchVelocity(out velocity))
        //     {
        //         Debug.LogError("Not enough speed to reach the target!");
        //         return;
        //     }
        //     StartCoroutine(MoveUntilReachTargetAndThenSetPositionToTarget());
        // }
        ...
        IEnumerator MoveUntilReachTargetAndThenSetPositionToTarget()
        {
            while (Vector3.Distance(transform.position, _targetPosition) > 0.1f)
            {
                transform.position = Vector3.MoveTowards(transform.position, _targetPosition, _shootingSpeed * Time.deltaTime);
                yield return null;
            }
